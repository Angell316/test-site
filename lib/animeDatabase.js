...
export function searchAnime(query, limit = 20) {
  ...
  const filtered = animeData.filter(anime => 
    ...
    anime.material_data?.title?.toLowerCase().includes(searchTerm) ||
    anime.material_data?.title_en?.toLowerCase().includes(searchTerm)
  )
  return removeDuplicates(filtered).slice(0, limit)
}
...
export function filterAnime({
  ...
  if (query) {
    ...
    filtered = filtered.filter(anime => 
      anime.title?.toLowerCase().includes(searchTerm) ||
      anime.title_orig?.toLowerCase().includes(searchTerm) ||
      anime.material_data?.anime_title?.toLowerCase().includes(searchTerm) ||
      anime.material_data?.title?.toLowerCase().includes(searchTerm) ||
      anime.material_data?.title_en?.toLowerCase().includes(searchTerm)
    )
  }
